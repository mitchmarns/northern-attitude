    ...
    return new Promise((resolve, reject) => {
      db.all(`
        ...
        WHERE p.visibility = 'public'
           OR p.character_id IN (SELECT followed_character_id FROM SocialFollowers WHERE follower_character_id = ?)
           OR p.character_id = ?
           OR (p.visibility = 'team' AND c.team_id = (SELECT team_id FROM Characters WHERE id = ?))
  ...
  getTeamPosts: async (characterId, page = 1, limit = 10) => {
    ...
    return new Promise((resolve, reject) => {
      db.all(`
        ...
        JOIN Characters c ON p.character_id = c.id
        LEFT JOIN Teams t ON c.team_id = t.id
        WHERE c.team_id = (SELECT team_id FROM Characters WHERE id = ?)
  ...
  getFollowingPosts: async (characterId, page = 1, limit = 10) => {
    ...
    return new Promise((resolve, reject) => {
      db.all(`
        ...
        JOIN Characters c ON p.character_id = c.id
        LEFT JOIN Teams t ON c.team_id = t.id
        WHERE p.character_id IN (SELECT followed_character_id FROM SocialFollowers WHERE follower_character_id = ?)
           AND (p.visibility = 'public' OR 
               (p.visibility = 'followers') OR
               (p.visibility = 'team' AND c.team_id = (SELECT team_id FROM Characters WHERE id = ?)))
        ORDER BY p.created_at DESC
        LIMIT ? OFFSET ?
  ...
  getSuggestedFollows: async (characterId, limit = 3) => {
    return new Promise((resolve, reject) => {
      db.all(`
        ...
        SELECT c.id, c.name, c.position, c.avatar_url, t.name as team_name
        FROM Characters c
        LEFT JOIN Teams t ON c.team_id = t.id
        WHERE c.id != ?
          AND c.id NOT IN (SELECT followed_character_id FROM already_following)
          -- Prioritize team members if character is on a team
          AND (c.team_id = (SELECT team_id FROM Characters WHERE id = ? AND team_id IS NOT NULL)
               OR c.id IN (
                 -- Get popular characters (those with many followers)
                 SELECT followed_character_id 
                 FROM SocialFollowers 
